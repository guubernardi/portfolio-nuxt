  ...
  <main class="servico">
    ...
    <section class="abertura">
      ...
      <h1 class="abertura__titulo">
        <span class="abertura__linha">Site para psicólogo</span>
        <span class="abertura__linha abertura__titulo--leve">que marca a primeira sessão.</span>
      </h1>

      <p class="abertura__texto">
        Uma página feita para quem chega em dúvida sair com a conversa aberta no WhatsApp.
        Escrita do zero, sem template, com o que a pessoa precisa ler antes de decidir.
      </p>
      ...
      <div class="abertura__acoes">
        ...
        <a class="acao acao--secundaria" href="#exemplos">Ver dois sites prontos</a>
      </div>
    </section>
    ...
    <section class="bloco">
      ...
      <h2 class="bloco__titulo revelar" v-revelar="90">
        <span class="bloco__linha">Quem procura terapia</span>
        <span class="bloco__linha bloco__titulo--leve">não está comparando preço.</span>
      </h2>
      <p class="bloco__texto revelar" v-revelar="170">
        Está se perguntando se o que sente é motivo suficiente, se terapia online funciona de
        verdade e se vai conseguir falar de coisa íntima com um estranho. Site de psicólogo
        costuma responder à pergunta errada: abre com abordagem, formação e técnica, que são
        exatamente as informações que importam depois. Antes disso, a pessoa precisa se
        reconhecer na tela.
      </p>

      <ul class="duvidas">
        <li
            v-for="(duvida, i) in duvidasDeQuemProcura"
            :key="duvida"
    ...
    <section class="bloco">
      ...
      <h2 class="bloco__titulo revelar" v-revelar="90">
        <span class="bloco__linha">Uma página inteira,</span>
        <span class="bloco__linha bloco__titulo--leve">pensada para o seu caso.</span>
      </h2>

      <ul class="entregas">
        <li
            v-for="(item, i) in entregas"
            :key="item.titulo"
    ...
    <section id="exemplos" class="bloco">
      <span class="bloco__rotulo revelar" v-revelar><b>03</b> Já no ar</span>
      <h2 class="bloco__titulo revelar" v-revelar="90">
        <span class="bloco__linha">Dois sites de psicóloga</span>
        <span class="bloco__linha bloco__titulo--leve">funcionando agora.</span>
      </h2>
      ...
      <ul class="exemplos">
        <li
          ...
          <NuxtLink class="exemplo__area" :to="`/projetos/${projeto.slug}`">
            <img
              :src="projeto.imagem"
              :alt="`${projeto.nome}, site para ${projeto.setor.toLowerCase()}`"
    ...
    <section class="fecho">
      <h2 class="fecho__titulo revelar" v-revelar>
        <span class="fecho__linha">Vamos montar</span>
        <span class="fecho__linha fecho__titulo--leve">o seu?</span>
      </h2>
      <p class="fecho__texto revelar" v-revelar="90">
        Manda uma mensagem contando como você atende hoje. Sem formulário longo e sem
        compromisso.
      </p>
...
const linkWhatsapp =
  ...
  encodeURIComponent('Olá! Sou psicólogo(a) e gostaria de um orçamento para um site.')
...
const exemplos = PROJETOS.filter((p) => p.slug && /psicolog/i.test(p.setor))

const duvidasDeQuemProcura = [
...
const canonica = urlCanonica('/site-para-psicologo')

const TITULO = 'Site para psicólogo'
const DESCRICAO =
      ...
      innerHTML: JSON.stringify({
        ...
        '@graph': [
          ...
          },
          {
            '@type': 'Service',
            '@id': `${canonica}#servico`,
            name: 'Criação de site para psicólogo',
            description: DESCRICAO,
            serviceType: 'Criação de site para psicólogo',
            provider: { '@id': ID_ORGANIZACAO },
            areaServed: { '@type': 'Country', name: 'Brasil' },
            audience: { '@type': 'Audience', audienceType: 'Psicólogos e psicólogas' },
            url: canonica,
          },
          {
            '@type': 'WebPage',
